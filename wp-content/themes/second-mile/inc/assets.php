...
if ( ! function_exists( 'secondmile_scripts' ) ) :
	...
	function secondmile_scripts() {
		...
		if ( is_page( 'who-we-are' ) ) :
			wp_register_script( 'who-we-are', get_template_directory_uri() . '/assets/js/who-we-are.js', array(), '1.1.0', true );
			wp_enqueue_script( 'who-we-are' );
		endif;
		
		if ( is_home() ) :
			wp_register_script( 'donate', get_template_directory_uri() . '/assets/js/donate.js', array(), '1.1.0', true );
			wp_enqueue_script( 'donate' );
		endif;

		if ( is_category() ) :
			wp_register_script( 'ministry-archive', get_template_directory_uri() . '/assets/js/ministry-archive.js', array(), '1.1.0', true );
			wp_enqueue_script( 'ministry-archive' );
		endif;

		if ( is_singular( 'post' ) ) :
			wp_register_script( 'ministry-single', get_template_directory_uri() . '/assets/js/ministry-single.js', array(), '1.1.0', true );
			wp_enqueue_script( 'ministry-single' );
		endif;

		if ( is_page( 'contact' ) ) :
			wp_register_script( 'contact', get_template_directory_uri() . '/assets/js/contact.js', array(), '1.1.0', true );
