...
function portfolio_item( $id ) {
  ...
  <div class="portfolio-item mb-8 relative">
    <a class="text-black" href="<?php echo get_the_permalink( $id ); ?>">
      <div class="aspect-16:9 bg-cover bg-no-repeat bg-center" style="background-image: url('<?php the_field( 'secondmile_project_featured_img', $id ); ?>');"></div>
  ...
  <li class="team-item relative lg:mb-8" id="team-member-<?php echo $id ?>" data-item-index="<?php echo $i; ?>" data-drawer-wrapper data-drawer-special data-drawer-expanded="false">
    <div class="relative">
      <div class="team-headshot relative">
        <img class="team-headshot-img" src="<?php the_field( 'secondmile_team_headshot', $id ); ?>" alt="<?php echo get_the_title( $id ); ?> headshot" />
        <div class="team-headshot-overlay flex lg:absolute lg:pin pt-4 pb-6 lg:py-0">
          <div class="flex-1 lg:absolute lg:pin-b lg:pin-l lg:pin-r lg:px-3 lg:pb-3">
            ...
            <p class="lg:mb-0 lg:text-white lg:leading-none"><?php the_field( 'secondmile_team_role', $id ); ?></p>
          </div>
      ...
      <div id="team-bio-<?php echo $id ?>" class="team-bio opacity-0 flex flex-col lg:flex-row justify-between items-start lg:items-end">
        <div class="mr-8 flex-1">
          ...
          <h4 class="team-bio-title hidden md:block mb-4"><?php the_field( 'secondmile_team_role', $id ); ?></h4>
        </div>
...
function news_item( $id, $is_featured = false ) {
  $author_id = get_field( 'secondmile_news_author', $id );
...
function job_content() {
  $secondmile_careers = new WP_Query( array(
    'post_type' => 'career',
    'post_status' => 'publish',
    'posts_per_page' => 50,
    'no_found_rows' => true,
  ...
  <div>
    <h3 class="mobile-heading uppercase sm:text-2xl mb-6"><?php the_field( 'secondmile_career_heading', 'options' ); ?></h3>
    <div class="leading-normal md:text-md mb-8"><?php the_field( 'secondmile_career_description', 'options' ); ?></div>
    <?php if ( $secondmile_careers->have_posts() ) : ?>
      <ul class="list-reset flex flex-col md:flex-row md:flex-wrap md:justify-between">
        <?php while ( $secondmile_careers->have_posts() ) : $secondmile_careers->the_post(); ?>
    ...
    <?php else : ?>
      <div class="no-careers"><?php the_field( 'secondmile_career_no_careers_description', 'options' ); ?></div>
    <?php endif; ?>
  </div>
