...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
  <div class="who-we-are-outer mt-nav relative">
    <div class="who-we-are-inner lg:w-3/5">
      <section class="pt-8">
        <div class="py-8 px-6">
        ...
        <?php if ( ! empty( $ministries ) ) : ?>
          <ul class="ministry-list flex flex-col list-reset text-white lg:w-2/5 lg:-my-3" data-fixed="false">
            <?php foreach ( $ministries as $index => $data ) : ?>
              <?php if ( ! empty( $data['title'] && ! empty( $data['excerpt'] ) ) ) : ?>
                <li class="ministry-list__item lg:flex-1 lg:flex lg:flex-col lg:justify-center lg:my-3 w-full bg-cover bg-center relative p-8 overflow-hidden" style="background-image: linear-gradient( rgba(<?php echo $data['color']; ?>, 0.83), rgba(<?php echo $data['color']; ?>, 0.83) ), url('<?php echo $data['bg']['url']; ?>');">
                  <p class="font-display leading-none absolute pin-t pin-l -mt-1 -ml-6 lg:-ml-8 opacity-25 ministry-list__item__count">0<?php echo $index + 1; ?></p>
                  <div>
      ...
      <?php if ( ! empty( $the_board ) ) : ?>
        <section class="bg-black text-white py-8">
          <h2 class="board-title font-display uppercase mb-6 -ml-3 tracking-wide">Board Of Directors</h2>
      ...
      <?php if ( ! empty( $our_dream ) ) : ?>
        <section class="p-8 text-center">
          ...
          <p class="py-8"><a class="button-outline button-outline--green" href="<?php echo $our_dream['page_link']; ?>"><?php echo $our_dream['page_link_text']; ?></a></p>
        </section>
      <?php endif; ?>
    </div>
  </div>
