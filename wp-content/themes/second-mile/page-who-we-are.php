...
$secondmile_featured_projects = new WP_Query( array(
  'post_type' => 'portfolio',
...
<section class="featured-projects md:h-screen pt-nav lg:pt-0" data-slider="featured">
  <div class="featured-projects-preview lg:opacity-0 flex flex-col-reverse lg:flex-row">
    <div class="featured-projects-info relative z-10 p-8 lg:p-0 lg:mt-0 lg:w-3/5">
      <div class="lg:absolute featured-projects-info--nav lg:px-8">
        <div class="slider-sub pt-4">
          <?php foreach( $secondmile_featured_projects->posts as $featured_project ) : ?>
            <div class="flex-col">
    ...
    <div class="featured-projects-slider slider w-full overflow-hidden relative">
      <?php foreach( $secondmile_featured_projects->posts as $featured_project ) : ?>
        <div class="featured-projects-img lg:hidden bg-cover bg-no-repeat bg-center h-full relative" style="background-image: url(<?php the_field( 'secondmile_project_featured_img', $featured_project->ID ); ?>);"><a href="<?php echo get_permalink( $featured_project->ID ); ?>" class="absolute"></a></div>
      <?php endforeach; ?>
    </div>
  </div>
